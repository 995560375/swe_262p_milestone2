...
public class XML {
     ...
     */
    public static boolean parse1(XMLTokener x, JSONObject context, String name, JSONPointer desiredPath, String currentPath, JSONObject replace, XMLParserConfiguration config)
            throws JSONException {
        char c;
        int i;
...
//            jsonObject = new JSONObject();
                    ...
                    for (;;) {
                        ...
                        if (token == null) {
                            ...
                            if(replace == null || desiredPath != null && !desiredPath.toString().equals(currentPath)) {
                                set = parse1(x, jsonObject != null ? jsonObject : context, tagName, desiredPath, currentPath, replace, config);
                            }
    ...
    private static boolean parseSearch(
        ...
        token = x.nextToken();
        System.out.println("token:" + token + " name:" + name + " currentPath: " + currentPath);
        ...
        if (token == BANG) {
            ...
            for (;;) {
                ...
                if (token instanceof String) {
                    ...
                    if (token == EQ) {
                        ...
                        if (config.isConvertNilAttributeToNull()
                            ...
                            nilAttributeFound = true;
                                ...
                                && TYPE_ATTR.equals(string)) {
                            xmlXsiTypeConverter = config.getXsiTypeMap().get(token);
                        } else if (jsonObject != null && !nilAttributeFound) {
                            jsonObject.accumulate(string,
                                    config.isKeepStrings()
                                            ? ((String) token)
                                            : stringToValue((String) token));
                    ...
                    for (;;) {
                        token = x.nextContent();
                        System.out.println("nextContent" + token);
                        if (token == null) {
//                            if (tagName != null) {
//                                throw x.syntaxError("Unclosed tag " + tagName);
//                            }
                            ...
                            string = (String) token;
                            if (jsonObject != null && string.length() > 0 ) {
                            ...
                            System.out.println("Before recursion, jsonObject: " + jsonObject + " context: " + context + " name: " + tagName + " currentPath: " + currentPath);
                            recursiveResult = parseSearch(x, jsonObject == null ? context : jsonObject, tagName, config, targetPath, currentPath);
                            System.out.println("recursiveResult " + recursiveResult + " jsonObject" + jsonObject + " name: " + tagName + " currentPath: " + currentPath);
                            if (jsonObject != null && context != null && recursiveResult) {
                                if (config.getForceList().contains(tagName)) {
                                    // Force the value to be an array
    ...
    private static boolean parseReplace(
            XMLTokener x,
            JSONObject context,
            String name,
            XMLParserConfiguration config,
        ...
        if (token == BANG) {
            ...
            currentPath += "/" + tagName;
            System.out.println("currentPath " + currentPath);
            // Replace the json object when the path equals.
                ...
                if (token instanceof String) {
                    ...
                    for (;;) {
                        ...
                        if (token == null) {
                            ...
                            // Nested element
                            boolean recursiveResult = true;
                            // Recurse when the paths are not equal
                            System.out.println("Before recursion, jsonObject: " + jsonObject + " name: " + tagName + " currentPath: " + currentPath);
                            if (!currentPath.equals(targetPath)) {
                                recursiveResult = parseReplace(x, jsonObject, tagName, config, targetPath, currentPath, replacement);
                            }
                            System.out.println("recursiveResult " + recursiveResult + " jsonObject" + jsonObject + " name: " + tagName + " currentPath: " + currentPath);
                            if (recursiveResult) {
                                if (config.getForceList().contains(tagName)) {
                                    ...
                                    if (jsonObject.length() == 0) {
                                        ...
                                        if (currentPath.equals(targetPath)) {
                                            System.out.println("currentPath equals, tag" + tagName + "jsonObject" + jsonObject);
                                            // TODO: replacement.get or jsonObject.get
    ...
    private static boolean parseAndModify(XMLTokener x, JSONObject context, String name, XMLParserConfiguration config, Function<String, String> changeString)
        ...
        Object token;
        XMLXsiTypeConverter<?> xmlXsiTypeConverter;
        Function<String, String> changeToNew = old -> changeString.apply(old);
            ...
            if (c == '-') {
                ...
                if ("CDATA".equals(token)) {
                    if (x.next() == '[') {
                        string = x.nextCDATA();
                        if (string.length() > 0) {
                            context.accumulate(changeToNew.apply(config.getcDataTagName()), string);
                        }
                        return false;
                    }
                }
            ...
            for (;;) {
                ...
                if (token instanceof String) {
                    ...
                    if (config.getForceList().contains(tagName)) {
                        // Force the value to be an array
                        if (nilAttributeFound) {
                            context.append(changeToNew.apply(tagName), JSONObject.NULL);
                        } else if (jsonObject.length() > 0) {
                            context.append(changeToNew.apply(tagName), jsonObject);
                        } else {
                            context.put(changeToNew.apply(tagName), new JSONArray());
                        }
                    } else {
                        if (nilAttributeFound) {
                            context.accumulate(changeToNew.apply(tagName), JSONObject.NULL);
                        } else if (jsonObject.length() > 0) {
                            context.accumulate(changeToNew.apply(tagName), jsonObject);
                        } else {
                            context.accumulate(changeToNew.apply(tagName), "");
                        }
                    }
                    return false;
                    ...
                    for (;;) {
                        ...
                        if (token == null) {
                            ...
                            // Nested element
                            if (parseAndModify(x, jsonObject, tagName, config, changeString)) {
                                if (config.getForceList().contains(tagName)) {
                                    // Force the value to be an array
                                    if (jsonObject.length() == 0) {
                                        context.put(changeToNew.apply(tagName), new JSONArray());
                                            ...
                                            && jsonObject.opt(config.getcDataTagName()) != null) {
                                        context.append(changeToNew.apply(tagName), jsonObject.opt(config.getcDataTagName()));
                                    } else {
                                        context.append(changeToNew.apply(tagName), jsonObject);
                                    }
                                } else {
                                    if (jsonObject.length() == 0) {
                                        context.accumulate(changeToNew.apply(tagName), "");
                                            ...
                                            && jsonObject.opt(config.getcDataTagName()) != null) {
                                        context.accumulate(changeToNew.apply(tagName), jsonObject.opt(config.getcDataTagName()));
                                    } else {
                                        context.accumulate(changeToNew.apply(tagName), jsonObject);
                                    }
                                }
...
//            x.skipPast("<");
//            if(x.more()) {
//                parse1(x, jo, null, path, "/", null, new XMLParserConfiguration());
//            }
//        }
//        return jo;
//    }
public static JSONObject toJSONObject(Reader reader, JSONPointer path, JSONObject replacement) {
    ...
    while (x.more()) {
        x.skipPast("<");
        if (x.more()) {
            parseReplace(x, jo, null, XMLParserConfiguration.ORIGINAL, path.toString(), "", replacement);
        }
    }
    return jo;
}
    ...
    public static JSONObject toJSONObject(Reader reader, JSONPointer path) {
        ...
        while (x.more()) {
            x.skipPast("<");
            if (x.more()) {
                parseSearch(x, jo, null, XMLParserConfiguration.ORIGINAL, path.toString(), "");
            }
        }
        return jo;
    }
    ...
    public static JSONObject toJSONObject(Reader reader, Function<String, String> changeString) throws JSONException {
        ...
        while (x.more()) {
            x.skipPast("<");
            if(x.more()) {
                parseAndModify(x, jo, null, XMLParserConfiguration.ORIGINAL, changeString);
            }
        }
        return jo;
    }
